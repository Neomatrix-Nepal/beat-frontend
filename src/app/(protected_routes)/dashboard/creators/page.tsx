...
import {
  ...
  fetchCreators,
} from "@/src/app/actions/creator-actions";
...
const CreatorsPage = () => {
  ...
  const fetchData = useCallback(async (page: number) => {
    ...
    setError(null);
    try {
      const response: FetchCreatorsResponse = await fetchCreators(
        page,
        ITEMS_PER_PAGE
      );
      setCreators(
    ...
    async (id: number) => {
      try {
        ...
        if (result.success) {
          ...
          showDeleteToast(
            "Error",
            result.error || "Failed to delete creator",
            "error"
          );
        }
      } catch (error) {
  ...
  const handleDeleteMultiple = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        showDeleteToast(
          "Success",
          "Selected creators deleted successfully",
          "success"
        );
        setSelectAll(false);
        fetchData(currentPage);
        ...
        showDeleteToast(
          "Error",
          result.error || "Failed to delete creators",
          "error"
        );
      }
    } catch (error) {
  ...
  const handleApproveCreator = useCallback(
    async (userId: number, rowId: number) => {
      try {
        ...
        if (result.success) {
          ...
          showUpdateToast(
            "Error",
            result.error || "Failed to approve creator",
            "error"
          );
        }
      } catch (error) {
