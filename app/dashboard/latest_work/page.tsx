...
const LatestWorkManager: React.FC = () => {
  ...
  const handleSelectAllWorks = useCallback(() => {
    setSelectAll((prev) => {
      ...
      setWorks((prevWorks) => prevWorks.map((work) => ({ ...work, selected: newSelectAll })));
      return newSelectAll;
    });
  }, []);
...
const handleBatchDelete = useCallback(async () => {
  ...
  if (confirm(`Are you sure you want to delete ${selectedIds.length} selected work(s)?`)) {
     try {
  ...
  const selectedCount = useMemo(() => works.filter((work) => work.selected).length, [works]);
