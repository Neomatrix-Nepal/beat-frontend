      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-[#273142] p-6 rounded-lg space-y-3">
          <div className="flex items-center gap-3">
            <img
              src="http://localhost:3000/_next/image?url=https%3A%2F%2Fi.pravatar.cc%2F150%3Fimg%3D1&w=96&q=75"
              alt="avatar"
          ...
          <p className="text-sm text-gray-400 flex items-center gap-2">
            <img src={headset.src} alt="genre" className="w-4 h-4" />
            <span className="font-medium text-white">Music Genre:</span> Hip-Hop
          </p>
          <p className="text-sm text-gray-400 flex items-center gap-2">
            <img src={editor.src} alt="style" className="w-4 h-4" />
            <span className="font-medium text-white">
              Preferred Mixing Style:
            </span>{" "}
            Crisp
          </p>
          <p className="text-sm text-gray-400 flex items-center gap-2">
            <img src={mic.src} alt="needs" className="w-4 h-4" />
            <span className="font-medium text-white">Music Needs:</span> I want
            something like Travis Scott’s style.
          </p>
          <p className="text-sm text-gray-400 flex items-center gap-2">
            <img src={link.src} alt="file upload" className="w-4 h-4" />
            <span className="font-medium text-white">File Upload:</span>
            <a href="#" className="text-[#74f9e0] underline">
              Open File
            </a>
          </p>
          <p className="text-sm text-gray-400 flex items-center gap-2">
            <img src={chat.src} alt="instructions" className="w-4 h-4" />
            <span className="font-medium text-white">
              Additional Instructions:
            </span>{" "}
            Add some echo in the second verse.
          </p>
        </div>
